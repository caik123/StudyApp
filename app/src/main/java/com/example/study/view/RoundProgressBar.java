...
/**
 * @author GhostKang
 * @description: 类似于常见app splash页右上角的进度条
...
public class RoundProgressBar extends View {
    ...
    private static final int RING_WIDTH = DensityUtil.dp2px(3);
    private static final int TEXT_SIZE = 10;
    private static final int TEXT_COLOR = Color.WHITE;
    private static final int WIDTH = DensityUtil.dp2px(35);
     ...
     */
    public void startSlide(final long countDown, final SlideCallback callback) {
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                for (int i = 0; i < mMaxProgress; i++) {
                    ...
                    setCurProgress(i + 1); // 设置当前进度，在 onDraw() 中通过这个去绘制进度
                    postInvalidate(); // 通知 View 进行绘制
                    // 将当前进度回调给调用方，调用方可根据当前进度来实现具体逻辑
                    // 使用 post（）是因为回调在主线程发出后，调用方就不用再去切换回主线程了
                    post(new Runnable() {
                        @Override
                        public void run() {
                            callback.onProgress(mCurProgress, mMaxProgress);
                        }
                    });
                }
            }
    ...
    public static void setSkip(boolean sIsSkip) {
        RoundProgressBar.sIsSkip = sIsSkip;
    }
